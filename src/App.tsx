...
const CONTRACT_ADDRESS = "0x843E6a68624A9e6be627F42ee74A28B3460fE6Ce";
...
function HomePage() {
  ...
  const connectWallet = async () => {
    ...
    ctr.on("FeeCollected", (user: string, amount: bigint) => {
      ...
      showToast(
        `Fee collected: ${feeAmount} tBNB from ${user.slice(
          0,
          6
        )}...${user.slice(-4)}`,
        "success"
  ...
  const handleDoubleIncrement = async () => {
    if (!contract) return;
    setLoading(true);
    try {
      showToast("Transaction submitted...", "info");
      const tx = await contract.doubleIncrement({
        value: ethers.parseEther("0.00005"), // Send 0.00005 tBNB fee
      });
      await tx.wait();
      ...
      console.error("Double increment error:", error);
      ...
      if (errorMessage.includes("Only admin can call this function")) {
        showToast(
          "❌ Access denied: Only admin can use double increment",
          "error"
        );
      } else {
  ...
  return (
    <div className="page-container">
      ...
      {loading && (
        <div style={{ marginTop: 16 }}>Waiting for transaction...</div>
      )}
      ...
      {transactions.length > 0 && (
        ...
        >
          <div
            style={{
              background:
                "linear-gradient(135deg, rgba(162, 89, 255, 0.1), rgba(100, 108, 255, 0.05))",
                ...
                <div
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.boxShadow =
                      "0 6px 20px rgba(162, 89, 255, 0.2)";
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background =
                      "linear-gradient(135deg, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.05))";
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: "48px",
                        borderRadius: "50%",
                        background: `linear-gradient(135deg, ${getTransactionColor(
                          tx.type
                        )}, ${getTransactionColor(tx.type)}80)`,
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        fontSize: "1.5rem",
                        boxShadow: `0 4px 12px ${getTransactionColor(
                          tx.type
                        )}40`,
                      }}
                    >
